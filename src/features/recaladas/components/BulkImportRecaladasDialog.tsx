...
import { recaladasApi, type BulkRecaladasResult } from "@/core/api"
...
export function BulkImportRecaladasDialog({ isOpen, onClose, onSuccess }: Props) {
  ...
  const [result, setResult] = useState<BulkRecaladasResult | null>(null)
        ...
        <div>
          ...
          <div className="flex gap-2">
            {(["CREATE_ONLY", "UPSERT"] as Mode[]).map((m) => (
              ...
              >
                {m === "CREATE_ONLY" ? "Solo crear nuevas" : "Crear y actualizar"}
              </button>
            ))}
          </div>
        </div>
        ...
        {result && (
          <div className="rounded-xl border border-[rgb(var(--color-border))] overflow-hidden">
            ...
            <div className="px-4 py-3 grid grid-cols-4 gap-3 text-center">
              {[
                { label: "Solicitadas", value: result.requested },
                { label: "Creadas", value: result.created },
                { label: "Actualizadas", value: result.updated },
                { label: "Fallidas", value: result.failed },
              ].map(({ label, value }) => (
                <div key={label}>
